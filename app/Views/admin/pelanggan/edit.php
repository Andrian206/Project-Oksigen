...
<?= $this->section('content') ?>
    ...
    <h1>Edit Pelanggan <?= esc($pelanggan['nama']) ?></h1>
    ...
    <form action="/admin/pelanggan/<?= $pelanggan['id_pelanggan'] ?>" method="post">
        ...
        <textarea name="alamat" id="alamat"><?= old('alamat', $pelanggan['alamat']) ?></textarea>

        <label for="no_hp">No. HP</label>
        <input type="text" name="no_hp" id="no_hp" value="<?= old('no_hp', $pelanggan['no_hp']) ?>">

        <div class="checkbox-group">
            <input type="checkbox" name="jaminan_tabung" value="1" id="jaminan_tabung" <?= old('jaminan_tabung', $pelanggan['jaminan_tabung']) ? 'checked' : '' ?>>
